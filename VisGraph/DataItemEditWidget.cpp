...
mbUpdateDisplay(false)
{
	...
	if (mType==DATA_ITEM_INT || mType==DATA_ITEM_FLOAT || mType==DATA_ITEM_SHORT){
		...
		mLineEdit->setGeometry(180, 0, 60, 20);
		mLineEdit->setFont(default_font);
		//slider
		mSlider = new QSlider(this);
		mSlider->setOrientation(Qt::Horizontal);
		mSlider->setGeometry(0, 20, 240, 20);
...
mbUpdateDisplay(false)
{
	if (enum_item.widget_type==DATA_ITEM_ENUM_RADIOBUTTON) {
		for (int i=0; i<enum_item.names.size(); ++i) {
			...
			button->setGeometry(0, i*20, 240, 20);
		...
		mComboBox->setGeometry(120, 0, 120, 20);
...
{
	QString trim_name = QString(mName.c_str()).split(".")[1];
	QFont font(QString("MS Shell Dlg 2"), 8);

	QPushButton* button = new QPushButton(this);
	button->setText(trim_name);
	button->setGeometry(0, 0, 240, 20);
	button->setFont(default_font);
	connect(button, SIGNAL(pressed()), this, SLOT(onEnumRadioButtonChanged()));
}
...
DataItemEditWidget::~DataItemEditWidget(){
	...
	mManager->removeDataItemEditWidget(mName, this);
}
...
void DataItemEditWidget::onSliderValueChanged(int value){
	...
	if (mbImmediateUpdate && !mbUpdateDisplay){
		onSliderRelease();
	}
}
...
void DataItemEditWidget::updateDisplayValue(){
	bool b_success = false;
	mbUpdateDisplay = true;
	if (mType == DATA_ITEM_INT) {
		int new_value = mManager->getIntValue(mName, b_success);
		if (new_value != mIntValue) {
			mSlider->setValue(new_value);
			mIntValue = new_value;
		}
	} else if (mType == DATA_ITEM_SHORT) {
		short new_value = mManager->getShortValue(mName, b_success);
		if (new_value != mShortValue) {
			mSlider->setValue(new_value);
			mShortValue = new_value;
		}
	} else if (mType == DATA_ITEM_FLOAT) {
		float new_value = mManager->getFloatValue(mName, b_success);
		if (new_value != mFloatValue) {
			mSlider->setValue((new_value - mMin) / (mMax - mMin) * 100);
			mFloatValue = new_value;
		}
	} else if (mType == DATA_ITEM_BOOL) {
		bool new_value = mManager->getBoolValue(mName, b_success);
		if (new_value != mBoolValue) {
			mCheckBox->setChecked(new_value);
			mBoolValue = new_value;
		}
	} else if (mType == DATA_ITEM_COLOR) {
		vec4f new_value = mManager->getColorValue(mName, b_success);
		if (new_value != mColorValue) {
			mColorPicker->setCurrentColor(QColor(new_value.r*255, new_value.g*255, new_value.b*255, new_value.a*255));
			mColorValue = new_value;
		}
	} else if (mType == DATA_ITEM_ENUM) {
		int new_value = mManager->getEnumValue(mName, b_success);
		if (new_value != mEnumValue) {
			if (!mRadioButtons.empty()) {
				mRadioButtons[new_value]->setChecked(true);
			} else {
				mComboBox->setCurrentIndex(new_value);
			}
			mEnumValue = new_value;
		}
	} else if (mType == DATA_ITEM_TRIGGER) {
		mManager->trigger(mName);
	}
