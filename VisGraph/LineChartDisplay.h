...
public:
	...
	void drawLines() {
		...
		DisplayWidget::glColor(mLineColor);
		for (int i = 0; i < mData.size(); ++i) {
			glBegin(GL_LINE_STRIP);
			for (int j = 0; j < mData[i].size(); ++j) {
				DisplayWidget::glVertex(getNodePosition(i, j));
			}
			glEnd();
		}
	}
...
public:
		...
		for (int i = 0; i < mData.size(); ++i) {
			...
			for (int j = 0; j < mData[i].size(); ++j) {
				if (inRange(mXRange, mData[i][j].x)) {
					float x = interpolate(mXRange.lower, mXRange.upper, mData[i][j].x, 0.0f, 1.0f);
					float y = interpolate(mYRange.lower, mYRange.upper, mData[i][j].y, 0.0f, 1.0f);
					DisplayWidget::glColor(ColorMap::getLinearColor(y * 100.0f, mColorScheme));
					vec2f p = origin + x * mArea.row_axis;
					DisplayWidget::glVertex(p);
					DisplayWidget::glVertex(p + col_axis);
				}
			}
			glEnd();
			origin += col_unit;
	...
	vec2f getColumnUnit() {
		return mArea.col_axis / mData.size();
	}
